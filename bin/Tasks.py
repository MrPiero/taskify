import json


class TaskManager:
    def __init__(self):
        self.tasks = []
        self.load()

    def load(self, file="tasks.json"):
        try:
            with open(file, "r", encoding="utf-8") as task_file:
                tasks_json = json.load(task_file)
            for task in tasks_json:
                self.tasks.append(Task(task["name"], task["priority"], task["steps"]))
            self.tasks = sorted(self.tasks, key=lambda k: k.priority, reverse=True)
        except FileNotFoundError:
            pass

    def save(self, file="tasks.json"):
        tasks_json = []
        for task in self.tasks:
            tasks_json.append(task.to_dicc())
        with open(file, "w", encoding="utf-8") as task_file:
            json.dump(tasks_json, task_file, ensure_ascii=False)

    def show(self):
        i = 0
        print()
        for task in self.tasks:
            print("\t", i + 1, ". ", task.name, "(", task.priority, ")")
            i += 1

    def assign(self, task=None):
        if task is None:  # For console
            print("\n*** Add Task ***")
            name = input("Name of the task?: ")
            try:
                priority = int(input("Priority of the task (1-->5): "))
            except ValueError:
                priority = 1
            finished = False
            steps = []
            while not finished:
                step = input("Add step #" + str(len(steps) + 1) + " (Enter empty to finish): ")
                if step != "":
                    steps.append(step)
                else:
                    finished = True
            self.tasks.append(Task(name, priority, steps))
            self.save()
            self.tasks = sorted(self.tasks, key=lambda k: k.priority, reverse=True)
            print("*"*16)
        else:
            self.tasks.append(task)
            self.save()

    def delete(self):
        print("\n*** Delete Task ***\n\nSelect a task index to delete:")
        self.show()
        while 1:
            try:
                i = int(input("\nIndex? (0 to cancel): ")) - 1
                if i >= 0:
                    print("Deleted task \"" + self.tasks.pop(i).name + "\".")
                    self.save()
                    break
                elif i == -1:
                    print("Deletion canceled. ")
                    break
                else:
                    raise IndexError
            except (ValueError, IndexError) as e:
                print("\n\"" + str(i+1) + "\" is not a valid task index.", type(e))
        print("*"*19)

    def action(self, option):
        try:
            i = int(option) - 1
            try:
                task = self.tasks[i]
                print("\n*** Steps for", task.name, "P" + str(task.priority), "***")
                s = 0
                for step in task.steps:
                    s += 1
                    print("\t", s, ". ", step)
                    input()
                print("*********************" + len(task.name)*"*")
            except IndexError:
                print("\n\"" + str(i) + "\" is not a valid task index.")
        except ValueError:
            if option in (":A", "A"):
                self.assign()
            elif option in (":D", "D"):
                self.delete()
            elif option in (":Q", "Q"):
                pass
            else:
                print("\n\"" + option + "\" is not a valid option.")


class Task:
    def __init__(self, name="def", priority=1, steps=["steps here", "steps there"]):
        self.name = name
        self.priority = priority
        self.steps = steps

    def to_dicc(self):
        return {"name": self.name, "priority": self.priority, "steps": self.steps}
